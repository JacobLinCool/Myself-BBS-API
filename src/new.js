...
async function get_new_animes() {
    ...
    let total = +[...page1.matchAll(total_regexp)][0][1] || 1;

    let item_regexp = new RegExp(
        `<em>\\[<a href="forum.php\\?mod=forumdisplay&fid=133&amp;filter=typeid&amp;typeid=(\\d+?)">([^]+?)<\\/a>]<\\/em> <a href="thread-(\\d+?)-1-\\d.html" onclick="atarget\\(this\\)" class="xst" >([^]+?)<\\/a>`,
        "g"
    );
